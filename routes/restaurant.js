...
router.get('/orders111', function(req, res, next) {
	sess = req.session;
    if(!sess.restaurant){
    	res.redirect('/restaurant');
    }
...
router.get('/orders', function(req, res, next) {
    ...
    var restaurant = sess.restaurant;
    connection.query("select * from RESTUARANT_MENUs_ORDERS orders JOIN RESTUARANT_MENUS menu ON orders.MENU = menu.RESTUARANT_MENU_ID AND  menu.ORDERDATE>='2017-05-18'  and menu.ORDERDATE<='2017-05-18' and menu.RESTUARANT_ID = ?",[restaurant], function(err, rows) {
		if(err)
                console.log("Error Selecting : %s ",err );
        console.log(rows);
        res.render('orders',{page_title:"Orders",data:rows});
...
router.get('/orders', function(req,d1,d2, res, next) {
	sess = req.session;
    ...
    var restaurant = sess.restaurant;
   connection.query("select * from RESTUARANT_MENUs_ORDERS orders JOIN RESTUARANT_MENUS menu ON orders.MENU = menu.RESTUARANT_MENU_ID AND menu.RESTUARANT_ID = ?   and menu.ORDERDATE >= ? and  menu.ORDERDATE <= ? ",[restaurant],[d1],[d2] function(err, rows) {
		if(err)
                console.log("Error Selecting : %s ",err );
        console.log(rows);
         console.log("select * from RESTUARANT_MENUs_ORDERS orders JOIN RESTUARANT_MENUS menu ON orders.MENU = menu.RESTUARANT_MENU_ID AND menu.RESTUARANT_ID = %s   and menu.ORDERDATE >= %s and  menu.ORDERDATE <= %s ",[restaurant],[d1],[d2] );
    
        res.render('orders',{page_title:"Orders with Date Specification",data:rows});
    });
});
