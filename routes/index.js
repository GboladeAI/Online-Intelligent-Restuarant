...
router.get('/myorders', function(req, res, next) {
    ...
    if (startdate != "" && enddate != "") {
        connection.query("select * from RESTUARANT_MENUs_ORDERS orders JOIN RESTUARANT_MENUS menu ON orders.MENU = menu.RESTUARANT_MENU_ID AND orders.customer = ?", [customer, startdate, enddate], function(err, rows) {
            if (err) console.log("Error Selecting : %s ", err);
            console.log(rows);
